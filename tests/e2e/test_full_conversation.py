...
async def test_full_conversation_flow(test_client, services):
    ...
    resp = await test_client.post("/api/chat", json={"session_id": session_id, "message": "Toyota"})
    ...
    resp = await test_client.post("/api/chat", json={"session_id": session_id, "message": "2020"})
...
async def test_duplicate_detection_flow(test_client, services):
    ...
    await test_client.post("/api/chat", json={"session_id": session_id, "message": "Jan de Vries"})
...
async def test_partial_conversation_then_close(test_client, services):
    ...
    await test_client.post("/api/chat", json={"session_id": session_id, "message": "It's a sedan"})
...
async def test_duplicate_then_complete_updates_existing_registration(test_client, services):
    ...
    await test_client.post("/api/chat", json={"session_id": session_id, "message": "1985-06-15"})
