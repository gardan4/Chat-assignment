...
async def test_session_includes_messages_history(test_client):
    ...
    await test_client.post("/api/chat", json={"session_id": session_id, "message": "Hello"})
...
async def test_chat_missing_message_field(test_client):
    ...
    response = await test_client.post("/api/chat", json={"session_id": session_id})
...
async def test_chat_missing_session_id_field(test_client):
    ...
    response = await test_client.post("/api/chat", json={"message": "hello"})
    assert response.status_code == 422
