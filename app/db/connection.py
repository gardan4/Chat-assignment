...
class MongoDB:
    ...
    async def connect(self, url: str, database: str, session_ttl: int = 3600):
        ...
        for candidate in urls:
            try:
                client = AsyncIOMotorClient(candidate, serverSelectionTimeoutMS=5000)
                await client.admin.command("ping")
                self.client = client
                logger.info("Connected to MongoDB at %s", candidate)
                break
            except Exception as exc:
                logger.warning("MongoDB '%s' unreachable: %s — trying next", candidate, exc)
                last_exc = exc
        else:
            raise ConnectionError(f"Could not connect to any MongoDB URL: {urls}") from last_exc
