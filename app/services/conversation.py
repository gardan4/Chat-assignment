...
class ConversationService:
    ...
    async def process_message(self, session_id: str, user_message: str) -> dict:
        ...
        instructions = (
            ...
            "\n- ALWAYS use the extract_customer_data tool when the "
            "customer provides or corrects any information."
            "\n- When the customer corrects a previously given value, "
            "call extract_customer_data with the updated value."
            "\n- When all information is collected and the customer "
            "confirms, call mark_registration_complete."
        )
        system_content = self.prompt_config.system_prompt + instructions
                ...
                if fn_name == "extract_customer_data":
                    try:
                        ...
                        if extracted:
                            ...
                            logger.info(f"Session {session_id}: extracted {list(extracted.keys())}")
                            await self._check_duplicate(session_id, extracted, schema)
                        tool_results.append(
                            {
                                "id": tool_call["id"],
                                "result": json.dumps(
                                    {"status": "ok", "saved": list(extracted.keys())}
                                ),
                            }
                        )
                    except json.JSONDecodeError:
                        logger.warning(
                            f"Failed to parse tool args: {tool_call['function']['arguments']}"
                        )
                        tool_results.append(
                            {
                                "id": tool_call["id"],
    ...
    async def _finalize_registration(self, session_id: str) -> None:
        ...
        if existing:
            ...
            await self.registration_repo.create(pii_hash, fields, self.prompt_config.schema_version)
